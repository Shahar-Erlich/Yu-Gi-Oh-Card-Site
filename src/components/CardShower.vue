<template>
    ...
    <div class="grid grid-cols-[370px_600px] ml-48 mt-24 gap-x-5">
        <!-- Card div -->
        <div class="flip-card inline-block">
            <div class="flip-card-inner" :class="{ flipped: isFlipped }">
                <div class="flip-card-front">
                    <img draggable="false"
                        class="transform transition-transform duration-300 hover:scale-125 w-16  md:w-32 lg:w-90 xl:w-96 z-10"
                        :src="this.cardImage" alt="/CardBack.jpg" @click="openModal">
                </div>
        ...
        <!-- Info div-->
        <div :class="{ 'opacity-0': !show, 'opacity-100': show }"
            class=" text-white transition-opacity duration-500 ease-in-out" style="text-align: left;">
            <h1 class="text-4xl font-bold font-custom1">{{ this.cardNameD }}</h1>
            <p>{{ this.cardLvl }}</p>
            <div>
                <p class="font-custom3 inline-block"><img draggable="false" class="w-7 h-7 inline-block"
                        v-if="this.attImg" :src="this.attImg">{{ this.cardATT }}</p>
...
export default {
    ...
    watch: {
        cardName(newValue) {
            this.cardRecieved(newValue); // Call your function with the new value
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        async fetchData() {
            fetch('/api/api/v7/cardinfo.php?fname=' + this.cardName)
                .then((response) => response.json())
                .then((data) => {
                    // console.log(data.data[0].name);
                    this.cardNameD = data.data[0].name;
                    ...
                    this.cardImage += this.cardNameD.replace(/[/\\?%*:|"<>]/g, '') + ".jpg";
                    this.selectedImage = this.cardImage;
                    console.log(this.cardImage);
                    this.flipCard();
                    this.show = true; // Fade in after flipping
                })
                .catch((error) => {
